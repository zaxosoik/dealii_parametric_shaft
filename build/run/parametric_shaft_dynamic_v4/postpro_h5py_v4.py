...
angles_index=[]
angles = []
angles_lin = np.linspace(angle_[0],angle[-1],1000)
for k in range(angles_lin.shape[0]):
    angle_temp = angles_lin[k]
    for i in range(angle_.shape[0]):
        if abs(angle_[i]-angle_temp)<0.001:
...
grid_z = griddata((x_coordinates2plot, angles2plot), max_stresses, (grid_x, grid_y), method='linear')
...
for k in range(x_.shape[0]):
    ...
    for i in range(x_coordinates.shape[0]):
        if abs(x_coordinates[i]-temp_x)< 1/100:
            #temp_y=0
            if norm_of_stress[i]>temp_y:
                temp_i=i
                temp_x = x_coordinates[i]
                temp_y = norm_of_stress[i]
    
    index_max.append(temp_i)
    y_max.append(temp_y)
    x_max.append(temp_x)
...
max_stresses = norm_of_stress[index_max]
x_coord_max_stress = np.array(x_coordinates[index_max])
#unique = np.unique(x_coord_max_stress,return_index=True)
#unique = unique[1][:]
#x_coord_max_stress = x_coord_max_stress[unique]
#max_stresses = max_stresses[unique]
x_coord_max_stress = x_coord_max_stress
max_stresses = max_stresses

##X_Y_Spline = make_interp_spline(x_coord_max_stress, max_stresses)
#x_coord_max_stress = x_
#max_stresses = X_Y_Spline(x_coord_max_stress)
