...
for i in range(angle_.shape[0]):
    if abs(angle_[i]-angle_temp)<0.01:
...
for k in range(angles_index.shape[0]-1):
    ...
    for i in range(angles_index[k+1]-angles_index[k]):
        if norm_of_stress_outer_temp[i]>temp_z or abs(x_coordinates_outer_temp[i]-temp_x)>0.1:
        #if abs(x_coordinates_outer_temp[i]-temp_x)>0.1  or norm_of_stress_outer_temp[i]>temp_z:#abs(angle[i]-temp_y )> 0.1:
        ...
        z_max_.append(temp_z)
        deform2plot_.append(temp_d)
    print(angles_index[k])
    #X_Y_Spline = make_interp_spline(x_max_, y_max_)
    X_ = np.linspace(min(x_max_), max(x_max_), 1000)
    
    Y_ = np.linspace(angles[k],angles[k],1000)
    X_temp = np.linspace(min(x_max_), max(x_max_), np.size(z_max_))
    X_Z_Spline = make_interp_spline(X_temp, z_max_)
    #X_ = np.linspace(min(temp_z), max(temp_z), 1000)
    Z_ = X_Z_Spline(X_)
    X_DEF_Spline = make_interp_spline(X_temp, deform2plot_)
    #X_ = np.linspace(min(temp_z), max(temp_z), 1000)
    DEF_ = X_DEF_Spline(X_)
    y_max.append(Y_)
    x_max.append(X_)
    z_max.append(Z_)
    deform2plot.append(DEF_)
...
grid_z = griddata((x_coordinates2plot, angles2plot), max_stresses, (grid_x, grid_y), method='cubic')
...
ax_carpet.plot_surface(grid_x, grid_y, grid_z, cmap='viridis')#rcount=rcount, ccount=ccount,facecolors=colors, shade=False)#, c= deform2plot,cmap='viridis')
...
for i in range(x_coordinates_.shape[0]):
    ...
    if norm_of_stress_[i]>temp_y :#or abs(x_coordinates_[i]-temp_x )> 0.01:
        
            temp_x = x_coordinates_[i]
            temp_y = norm_of_stress_[i]
    #elif abs(x_coordinates_[i]-temp_x )> 1.5:
...
for i in range(x_coordinates__.shape[0]):
    ...
    if abs(deformation_[i])>temp_y or abs(x_coordinates__[i]-temp_x )> 0.1:
...
ax2.plot(x_coord_max_stress, max_deform, color=color)
