          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            ...
            for (unsigned int face = 0; face < n_faces_per_cell; ++face)
            {
              ...
              //if (cell->boundary_id() == 1)
                  ...
                  else if (cell->face(face)->at_boundary() && (cell->face(face)->boundary_id() == 0))
              //if (cell->boundary_id() == 1)
                  {
...
//
    ...
    else
    {
          ...
          PETScWrappers::MPI::Vector distributed_incremental_solution(locally_owned_dofs, mpi_communicator);
