 ...
 //TODO CHECK THE BOUNDARIES
  ...
  void TopLevel<dim>::create_coarse_grid()
  {
    ...
    if (prm.change_diameter)
     {
      change_diameter(prm.x_1,prm.x_2,prm.radius, prm.radius_final);
      triangulation.execute_coarsening_and_refinement();
     }
    //
    if (prm.remove_hanging_nodes)
     GridTools::remove_hanging_nodes(triangulation,true,50);
    //GridTools::remove_anisotropy (triangulation,5,1);
   // refine_big_cells();
...
//
 ...
 //   //   }
 //   GridGenerator::subdivided_cylinder(triangulation_engine_shaft, prm.partitions, 0.99*radius, half_length);
...
//
 ...
 void TopLevel<dim>::compute_dirichlet_constraints()
 {
    ...
    if (prm.dynamicmode)
    {
        ...
        //                                        1
        //                                        ,
        //////                                       /*Functions::ZeroFunction<dim>(dim)*/ DisplacementEngine<dim>(prm.rpm*rpm_to_rps,prm.delta_t),
        //////                                        constraints_dirichlet_and_hanging_nodes,  component_mask);
        //////////VectorTools::interpolate_boundary_values(dof_handler,
        //////////                                        1
        ////////                                        ,
              ...
              for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
                {
                  ...
                  cell_stiffness_matrix(i, j) +=  (1+prm.HHTa)*1/(prm.beta*prm.delta_t*prm.delta_t)* cell_mass_matrix(i, j) +   (1+prm.HHTa)*prm.gamma/(prm.beta*prm.delta_t)*(cell_damping_matrix(i, j)) + /*(1+prm.HHTa)*/prm.beta*prm.delta_t*prm.delta_t*cell_matrix(i, j);     
...
//
    ...
    //system_mass_matrix *= prm.rho;
    system_mass_matrix.compress(VectorOperation::add);

    system_damping_matrix.compress(VectorOperation::add);
    //system_damping_matrix.add(prm.mu_rayleigh,system_mass_matrix);
...
//
  ...
  void TopLevel<dim>::time_stepping(const PETScWrappers::MPI::Vector &du)
  {
   ...
   // sol_trans.deserialize(distributed_vector);
    ...
    acceleration_n.add(1/(prm.beta*prm.delta_t*prm.delta_t),du_tmp);
    acceleration_n.add(-1/(prm.beta*prm.delta_t),velocity_n_prev);
    acceleration_n.add(-(1/(2*prm.beta)-1),acceleration_n_prev);
...
//
  ...
  void TopLevel<dim>::assemble_rhs_newton(const double a_HHT)
  {
   ...
   // pcout << "      Vector system_rhs Calced." <<std::endl; 
    ...
    system_mass_matrix.vmult(tmp,displacement_n_prev);
    newton_rhs.add(1/(prm.beta*prm.delta_t*prm.delta_t),tmp);
    tmp.reinit(locally_owned_dofs, mpi_communicator);

    system_mass_matrix.vmult(tmp,velocity_n_prev);
    newton_rhs.add(1/(prm.beta*prm.delta_t),tmp);
    tmp.reinit(locally_owned_dofs, mpi_communicator);
    
    system_mass_matrix.vmult(tmp,acceleration_n_prev);
    newton_rhs.add(1/(2*prm.beta)-1,tmp);
    tmp.reinit(locally_owned_dofs, mpi_communicator);
    system_damping_matrix.vmult(tmp,displacement_n_prev);
    newton_rhs.add(prm.gamma/(prm.beta*prm.delta_t),tmp);
    tmp.reinit(locally_owned_dofs, mpi_communicator);
    system_damping_matrix.vmult(tmp,velocity_n_prev);
    newton_rhs.add(prm.gamma/(prm.beta)-1,tmp);
    tmp.reinit(locally_owned_dofs, mpi_communicator);
    system_damping_matrix.vmult(tmp,acceleration_n_prev);
    newton_rhs.add((prm.gamma/(prm.beta)-2)*prm.delta_t/2,tmp);
