 ...
 void TopLevel<dim>::compute_dirichlet_constraints()
 {
  ...
  void TopLevel<dim>::assemble_system()
  {
   ...
   // double bearing_area = calculate_boundary_area(dof_handler,3);
    auto [total_area_prop, boundary_centroid] = calculate_boundary_area_and_centroid(dof_handler, 3);
   ...
   // pcout <<  "\n Total Area of Tailbearing: "<< total_area_prop << std::flush;
    ...
    pcout <<  "\n        Area of journal bearing spring: "<< journal_bearing_spring_area << " with centroid=  " << journal_bearing_spring_centroid <<std::flush;
    ...
    double mass_moment_of_inertia = calculate_mass_moment_of_inertia<dim>(
    dof_handler, density_function, -prm.half_length, prm.half_length,quadrature_formula);
                ...
                for (unsigned int q_point = 0; q_point < fe_face_values.n_quadrature_points;++q_point)
                    {
                        ...
                        if(point(dim-1)<=max_z_spring && point(dim-1)>=min_z_spring )
                        {
                            for (unsigned int i = 0; i < dim; ++i)
                                {
                                    rhs_values[i][i] = 5000000000/boundary_area*journal_bearing_spring_values[q_point][i];
                                }
                            for (unsigned int i = 0; i < dofs_per_cell; ++i)
                            for (unsigned int j = 0; j < dofs_per_cell; ++j)
