                                  ...
                                  const unsigned int       q_point)
{
                                               ...
                                               Vector<double> &values) const
  {
    ...
    //  values(direction) = 0; //present_timestep*velocity;
    //else
    //  values(direction) = displacement;
    //
  }
...
const SymmetricTensor<4, dim> TopLevel<dim>::stress_strain_tensor =
  get_stress_strain_tensor<dim>(lambda,
    ...
    double rpmToTorque(double rpm, double C) {
        ...
        double torque;
        if (rps>0)
           torque = power / (2 * pi * rps); // Calculate torque
        else
           torque = 0;
        return torque;
    }
...
//
 ...
 void TopLevel<dim>::compute_dirichlet_constraints()
 {
   ...
   // Vector<double> boundary_1();
  {
    ...
    const double prop_weight_pressure = prm.prop_weight/total_area_prop;
   ...
   // if (present_timestep> 1)
      ...
      std::map<types::global_dof_index, double> boundary_values;
       ...
       if (timestep_no==1)
             VectorTools::interpolate_boundary_values(dof_handler,
                                                4,
                                                 IncrementalBoundaryValues<dim>(0
                                                                        , present_timestep,prm.direction,prm.displacement),
                                                boundary_values,
      ...
      ////VectorTools::interpolate_boundary_values(dof_handler,
      ////                              4,
      ////                              Functions::ConstantFunction<dim>(prm.displacement,),
      ////                              boundary_values,
      ////                              fe.component_mask(z_component));
...
//
                  ...
                  for (unsigned int i = 0; i < dofs_per_cell; ++i)
                    {
                      ...
                      for (unsigned int j = 0; j < dofs_per_cell; ++j)
                      {
                        ...
                        cell_matrix(i, j) += eps_phi_j *rhs_h_values_m[component_i]/(12*prm.viscocity*1e-3)*((eps_phi_i)) *                    
                                              fe_face_values.JxW(q_point);
                      }
    ...
    pcout << "              Bearing norm of rhs is " << bearing_rhs.l2_norm() << std::endl;
    {
      ...
      SolverControl solver_control(20000, prm.bearing_tol);
      
      PETScWrappers::SolverGMRES cg(solver_control, mpi_communicator);
      ...
      total_bearings_locally_relevant_solution.add(1,distributed_locally_relevant_solution);
    }
...
//
  ...
  void TopLevel<dim>::assemble_rhs_newton(const double a_HHT)
  {
    ...
    newton_res_prev.compress(VectorOperation::add);
    newton_res.compress(VectorOperation::add);
    ...
    newton_rhs.add(-(1+a_HHT),newton_res);
    pcout << "      Vector newton_res Calced." <<std::endl;

    newton_rhs.add(a_HHT,newton_res_prev);
    pcout << "      Vector newton_res_prev Calced." <<std::endl;
    ...
    if (!prm.useReynolds)
        {
          ...
          for (unsigned int i = 4; i <7+prm.bearings; i++)
            { 
              ...
              VectorTools::interpolate_boundary_values(dof_handler,
                                                    4,
                                                    IncrementalBoundaryValues<dim>(5/*prm.end_time*/, present_timestep,prm.direction,prm.displacement),
                                                    boundary_values,
                                                    fe.component_mask(z_component));
            }
              MatrixTools::apply_boundary_values(
                      boundary_values, system_matrix, tmp_2, system_rhs, false);
              locally_relevant_solution =tmp_2;
        }
      else
         locally_relevant_solution = tmp;
    ...
    //MatrixTools::apply_boundary_values(
    //  boundary_values, system_dynamic_matrix, tmp, newton_rhs, false);
    ...
    for (unsigned int newton_step = 1; newton_step <= max_newton_iter; ++newton_step)
      {
        ...
        residual_norm = newton_res.l2_norm();
    ...
    if (prm.dynamicmode)
      {
         PETScWrappers::SolverGMRES cg(solver_control, mpi_communicator);
          ...
          //displacement_locally_relevant_solution.add(-1,displacement_n_prev);
      }
    else
    {
...
//
 ...
 //
  ...
  PETScWrappers::MPI::Vector total_displacement_out(locally_owned_dofs, mpi_communicator);
  ...
  void TopLevel<dim>::refine_initial_grid()
  {
    ...
    parallel::distributed::GridRefinement::refine_and_coarsen_fixed_number(
      triangulation, error_per_cell, 0.2, 0.03);
    triangulation.execute_coarsening_and_refinement();
