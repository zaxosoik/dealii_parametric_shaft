                                              ...
                                              for (unsigned int i = 0; i < dofs_per_cell; ++i)            
                                                {
                                                  ...
                                                  cell_displacement(i) +=  fe_face_values[displacement].value(i, q_point)*disp_rhs_values[i]*fe_face_values.JxW(q_point);
                                                  cell_velocity(i) += fe_face_values[displacement].value(i, q_point)*vel_rhs_values[i]*fe_face_values.JxW(q_point);
                                                }
                                             }
                                 }
...
//
    ...
    if (prm.dynamicmode)
    {
        ...
        VectorTools::interpolate_boundary_values(dof_handler,
                                            ...
                                            boundary_values) ;                                     
       // MatrixTools::apply_boundary_values(
       //   boundary_values, system_dynamic_matrix, acceleration_n, system_rhs, false);
//
...
//
    ...
    hanging_node_constraints.distribute(displacement_n);
    hanging_node_constraints.distribute(velocity_n);
    hanging_node_constraints.distribute(acceleration_n);
  ...
  {
    TimerOutput::Scope t(computing_timer, "solve_linear_problem");
    PETScWrappers::MPI::Vector distributed_incremental_solution(
      locally_owned_dofs, mpi_communicator);
 

          
    SolverControl solver_control(prm.max_iter*100,
